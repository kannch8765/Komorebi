#!/usr/bin/env python3
"""Komorebi personal info guard — pre-commit hook.
...
Checks three classes of violation:
  ...
  2. Personal info in source files — private nicknames or AI product
     names. Excludes the guard script itself.
...
Run via `.pre-commit-config.yaml`:
    ...
    - repo: local
      hooks:
        - id: komorebi-guard
          name: Komorebi personal info + author guard
          entry: python scripts/pre_commit_hooks/komorebi_guard.py
          language: system
          types: [file]
...
# AI product / brand names that should not appear in Komorebi's source code.
# These are fine in design docs (CLAUDE.md / PLAN.md may reference them for
# context) but should not leak into runtime code.
...
EXCLUDED_FILES: frozenset[str] = frozenset(
    {
        "scripts/pre_commit_hooks/komorebi_guard.py",  # the guard itself
    }
)
...
def check_author() -> list[str]:
    """Verify git config user.name and user.email match the Komorebi owner."""
    errors: list[str] = []
...
def main() -> int:
    ...
    if all_errors:
        print("=" * 64)
        print("KOMOREBI PERSONAL INFO GUARD: COMMIT BLOCKED")
        print("=" * 64)
