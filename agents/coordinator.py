...
def create_coordinator(
    ...
    return Agent(
        ...
        instruction=(
            ...
            "For vague queries like 'おすすめの場所' / 'recommendations' WITHOUT a specific "
            "place_type, default to 'cafe' and delegate immediately — do NOT ask for "
            "clarification.\n"
            "- MULTI (e.g. '渋谷に行って今日の天気は？' / 'Shibuya cafe + weather + route') → "
            "call all relevant sub-agents, then synthesize a combined answer\n"
            "- CROWDING keywords ('混み' / '混雑' / '人多い' / '空いてる' / 'crowded' / 'busy' / "
            "'how crowded') → ALWAYS delegate to route_agent first to get a real crowding_score; "
            "do NOT answer with general knowledge alone.\n"
            "- TIME-BUDGET RULE: When the user specifies a time budget "
            "('時間内' / '分以内' / '分で' / '時間で' / '以内' / 'N分' / 'N時間' / "
            "'within X minutes' / 'takes less than') WITHOUT a specific destination:\n"
            "    Step 1: Call places_agent to find candidate places matching the user's "
            "preference (のんびり / カフェ / 公園 etc.).\n"
            "    Step 2: For each candidate, call route_agent to check travel time from "
            "the user's origin (home if set).\n"
            "    Step 3: Return ONLY candidates where travel time fits the budget. "
            "Drop the rest.\n"
            "  If NO candidates fit the budget, tell the user and suggest a larger "
            "time range or closer areas.\n"
