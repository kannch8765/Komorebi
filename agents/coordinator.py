...
def create_coordinator(
    ...
    return Agent(
        ...
        instruction=(
            ...
            "- HARD RULE — TIME BUDGET: If the query contains ANY time-budget token "
            "('時間内' / '分以内' / '分で' / '時間で' / '以内' / 'N分' / 'N時間' / "
            "'within X minutes' / 'takes less than'), you MUST call route_agent FIRST "
            "to verify which destinations are reachable within the budget. Only THEN "
            "call places_agent around the verified destination area. Time budget is "
